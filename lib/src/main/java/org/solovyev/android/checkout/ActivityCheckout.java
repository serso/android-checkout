 ...
 * 		// ...
 * 		checkout.start();
 * 		// purchase flow is creates here as activity might be destroyed during purchase process (and
 * with it all the purchase flows are destroyed).
...
public final class ActivityCheckout extends Checkout {
    ...
    /**
     * Creates a permanent purchase flow with purchase <var>listener</var> to wait for purchase
     * updates. Listener will receive updates only from
     * purchase which was started with <var>requestCode</var>.
     * All purchase flows are automatically destroyed in {@link #stop()} method.
     * Permanent purchase flows are not destroyed when are finished (comparing to "one-shot"
     * purchase flows), thus,
     * <var>listener</var> methods might be called several times if several purchases with same
     * <var>requestCode</var>
     * are done.
     *
     ...
     * <var>listener</var>. As soon as purchase flow
     * is finished it is destroyed and <var>listener</var> is unregistered. Next purchase should be
     * done with a new purchase flow. This might
     ...
     * and {@link ActivityCheckout#getPurchaseFlow()} while starting purchase flow only this method
     * might be used:
     * <pre>
     * {@code
     * 	protected void onCreate(Bundle savedInstanceState) {
     ...
     * called with the same requestCode during
     ...
     * </pre>
     * <p/>
     * See {@link ActivityCheckout#createPurchaseFlow(int, RequestListener)} for adding the
     ...
     *
     * @return true if activity result was handled (there exists purchase flow for
     * <var>requestCode</var>)
